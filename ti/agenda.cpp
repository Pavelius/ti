...
INSTDATA(agendai) = {{"ArchivedSecret", "Утерянный секрет", Player},
...
void playeri::create_agenda_deck() {
	agenda_deck.clear();
	for(unsigned i = 0; i < bsdata<agendai>::source.getcount(); i++) {
		auto m = bsdata<agendai>::elements[i].count;
		if(!m)
			m = 1;
		...
		while(!all_coorrect) {
			...
			if(data.count >= 2) {
				sb.adds("Следующие два закона, которые просит поставит на голосование сенат это [%1] и [%2].",
					bsdata<agendai>::elements[data[0]].name,
					bsdata<agendai>::elements[data[1]].name);
			...
			for(unsigned i = 0; i < data.count; i++)
				ai.add(4 + i, "Отменить голосование %1", bsdata<agendai>::elements[data[i]].name);
