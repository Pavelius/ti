	...
	for(auto p : source) {
		if(!p->iscomputer()) {
			...
			for(auto e : politics)
				ai.add(e, getstr(e));
			ai.sort();
			p->strategy = (strategy_s)ai.choose("Эта стратегическая фаза. Вам нужно выбрать *одну* стратегию из списка ниже, которую будете использовать на этот ход. Будьте внимательны. Ваши враги также выбирают одну стратегию из этого же списка.");
		} else {
...
static void strategy_primary_action(player_info* p, strategy_s id) {
	...
	switch(id) {
	case Diplomacy:
		sb.add("Дипломатическая стратегия подразумевает, что вы достигаете определенных договоренностей с вашим врагом. Не он, не вы, не сможете нападать друг на друга.");
		if(!p->iscomputer())
			sb.adds("Кто станет вашим союзником на этот ход?");
